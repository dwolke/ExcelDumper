...
class OleReader
{
  ...
  public function read($file)
  {
    ...
    if (!is_readable($file)) {
      throw new \RuntimeException('Datei ist nicht lesbar.', 1);
    }

    $this->fileData = @file_get_contents($file);

    if (!$this->fileData) {
      throw new \RuntimeException('Fehler beim einlesen der Datei.', 1);
    }

    if (substr($this->fileData, 0, 8) != $identifierOle) {
      throw new \RuntimeException('Datei ist nicht lesbar oder hat ein falsches Format.', 1);
    }
