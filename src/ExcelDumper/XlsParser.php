...
use ExcelDumper\OleReader;
use ExcelDumper\ReaderUtil;
...
class XlsParser
{
  ...
  /**
   * Standardformate für Datum in Excel
   * @var array
   */
   ...
   * 
   * @param string $fileName die zu lesende Datei
   */
  private function read($file)
  {

    $res = $this->oleStorage->read($file);
            ...
            if ($indexCode > 0) {
              ...
              if ($formatstr != '') {
                ...
                if (preg_match("/[^hmsday\/\-:\s\\\,AMP]/i", $tmp) == 0) { // found day/time format
                  ...
                  $formatstr = trim(str_replace(array('am/pm', 'mmmm', 'mmm'), array('', 'm', 'm'), $formatstr));
                  ...
                  $formatstr = str_replace(array('dddd', 'ddd', 'dd', 'yyyy', 'yy', 'hh', 'h'), array('l', 'D', 'd', 'Y', 'y', 'H', 'H'), $formatstr);
